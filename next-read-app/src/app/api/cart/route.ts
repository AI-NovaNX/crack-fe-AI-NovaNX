...
export async function DELETE(request: NextRequest) {
  ...
  if (!token) return json({ message: "Please sign in again to remove this item." }, 401);
...
export async function POST(request: NextRequest) {
  ...
  if (request.headers.get("origin") !== request.nextUrl.origin)
    return json({ message: "Invalid origin" }, 403);
  const token = (await cookies()).get("nexread_access")?.value;
  if (!token) return json({ message: "Please sign in to add books to your cart." }, 401);
  const body = await request.json().catch(() => null);
...
export async function GET() {
  const headers = { "Cache-Control": "no-store" };
  const token = (await cookies()).get("nexread_access")?.value;
  if (!token) return NextResponse.json({ message: "Please sign in to view your cart." }, { status: 401, headers });
  try {
    ...
    return NextResponse.json({ message: error instanceof ApiError ? error.message : "The cart could not be loaded." }, { status: error instanceof ApiError ? error.status : 502, headers });
  }
}
