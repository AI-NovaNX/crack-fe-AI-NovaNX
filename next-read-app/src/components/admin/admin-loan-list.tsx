  ...
  return (
    <>
      ...
      <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
        <h1 id="borrowed-list-title" className="text-2xl font-extrabold tracking-tight sm:text-3xl">
          Borrowed List
        </h1>
      ...
      <div className="mt-3 flex flex-wrap gap-2" aria-label="Filter loans by status">
        {filters.map((item) => (
          <button
            key={item.value}
            type="button"
      ...
      <div className="mt-5 space-y-3">
        {loading ? (
          Array.from({ length: 3 }, (_, index) => (
            <div key={index} className="h-40 animate-pulse rounded-[24px] border border-palette-indigo-300-20 bg-gray-200" />
          ))
        ) : error ? (
          <div className="rounded-[24px] border border-red-400/20 bg-red-400/5 px-6 py-12 text-center text-sm text-red-300">
            <p>{error}</p>
            <button type="button" onClick={() => setAttempt((value) => value + 1)} className="mt-4 rounded-full border border-red-300/30 bg-red-400/10 px-4 py-2 text-xs font-bold hover:bg-red-400/20">
              Try again
            </button>
          </div>
        ) : loans.length === 0 ? (
          ...
          loans.map((loan) => {
            ...
            return (
              <article key={loan.id} className="rounded-[24px] border border-palette-indigo-300-20 bg-gray-200 p-4 shadow-[0_20px_50px_-30px_rgba(0,0,0,0.7)] sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-2 text-[9px]">
                  <span className="flex items-center gap-2 font-semibold text-palette-slate-400">
                    Status
                    <span className={cn("rounded-full border px-2.5 py-1 font-bold", statusStyles[status])}>
                      {formatStatus(status)}
                    </span>
                  </span>
                  <span className="text-palette-slate-400">
                    Due Date <strong className="ml-2 rounded-full bg-white/5 px-2.5 py-1 text-palette-slate-50">{formatDate(loan.dueAt)}</strong>
                  </span>
                </div>
                ...
                <div className="mt-4 grid grid-cols-[72px_minmax(0,1fr)] items-center gap-4 sm:grid-cols-[82px_minmax(0,1fr)_170px]">
                  ...
                  <div className="min-w-0">
                    ...
                    <h2 className="mt-2 truncate text-sm font-extrabold sm:text-base">{loan.book.title}</h2>
                    <p className="mt-1 text-xs text-palette-slate-400">{loan.book.author?.name || "Unknown author"}</p>
                    <p className="mt-3 text-[10px] font-semibold text-palette-slate-400">
                      {formatDate(loan.borrowedAt)}{duration ? ` · Duration ${duration} Days` : ""}
                    </p>
                  </div>
                  <div className="col-span-2 rounded-2xl border border-palette-indigo-300-20 bg-white/5 px-4 py-3 text-left sm:col-span-1 sm:text-right">
                    <p className="font-mono text-[8px] tracking-[0.18em] text-palette-slate-400 uppercase">Borrower Name</p>
                    <p className="mt-1 truncate text-xs font-extrabold">{loan.user.fullName}</p>
                    <p className="mt-1 truncate text-[9px] text-palette-slate-400">{loan.user.email}</p>
                  </div>
                </div>
              </article>
            );
      ...
      {!loading && !error && meta.totalPages > 1 && (
        <nav aria-label="Borrowed list pagination" className="mt-5 flex items-center justify-end gap-3 text-xs">
          <button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)} className="pagination-button disabled:cursor-not-allowed disabled:opacity-40">Previous</button>
          <span className="text-palette-slate-400">Page {meta.page} of {meta.totalPages}</span>
          <button type="button" disabled={page >= meta.totalPages} onClick={() => setPage((value) => value + 1)} className="pagination-button disabled:cursor-not-allowed disabled:opacity-40">Next</button>
        </nav>
      )}
    </>
  );
