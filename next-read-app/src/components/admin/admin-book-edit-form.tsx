...
export function AdminBookEditForm({ book }: { book: BookDetail }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(body?.message || "Book belum dapat diperbarui.");
      toast({
        title: "Book berhasil diperbarui",
        description: title,
        variant: "success",
      });
      ...
      const message =
        saveError instanceof Error
          ? saveError.message
          : "Book belum dapat diperbarui.";
      setError(message);
      toast({ title: "Gagal memperbarui book", description: message, variant: "error" });
    } finally {
      setSaving(false);
    }
  }
    ...
    >
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-xs font-bold text-palette-slate-400">
          Rating
          <input required type="number" min="0" max="5" step="0.1" value={rating} onChange={(event) => setRating(event.target.value)} className={fieldClass} />
        </label>
        <label className="text-xs font-bold text-palette-slate-400">
          Page Count
          <input type="number" min="1" value={pageCount} onChange={(event) => setPageCount(event.target.value)} className={fieldClass} />
        </label>
        <label className="text-xs font-bold text-palette-slate-400">
          Total Copies
        ...
        <label className="text-xs font-bold text-palette-slate-400">
          Cover URL
          <input value={coverUrl} onChange={(event) => setCoverUrl(event.target.value)} className={fieldClass} />
        </label>
        <label className="text-xs font-bold text-palette-slate-400 sm:col-span-2">
          Description
          <textarea value={description} maxLength={5000} onChange={(event) => setDescription(event.target.value)} className={`${fieldClass} min-h-40 resize-y py-3`} />
        </label>
      </div>
      {error && <p role="alert" className="mt-5 text-sm text-red-400">{error}</p>}
      <div className="mt-7 flex justify-end gap-3">
        <Link href={`/admin/books/${encodeURIComponent(book.id)}`} className="rounded-full border border-border bg-secondary px-6 py-3 text-sm font-bold">Cancel</Link>
        <button type="submit" disabled={saving} className="rounded-full bg-gradient-to-r from-cyan-400 to-violet-600 px-6 py-3 text-sm font-extrabold text-white disabled:opacity-50">
