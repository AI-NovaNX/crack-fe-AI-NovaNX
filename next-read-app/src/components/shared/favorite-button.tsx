...
export function FavoriteButton({ book, showLabel = false }: { book: Book; showLabel?: boolean }) {
  ...
  return <button type="button" aria-label={`${label}: ${book.title}`} aria-pressed={saved} title={label} disabled={status === "loading"} onClick={event => {
    ...
    try { toggle(book); toast({ title: saved ? "Buku dihapus dari favorit" : "Buku ditambahkan ke favorit", description: book.title, variant: "success" }); }
    catch (e) { toast({ title: "Favorit belum dapat diperbarui", description: e instanceof Error ? e.message : "Penyimpanan browser tidak tersedia.", variant: "error" }); }
