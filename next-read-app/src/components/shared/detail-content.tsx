...
export function DetailContent({ book, related, relatedError }: { book: BookDetail; related: Book[]; relatedError: boolean }) {
  ...
  return <>
    <BookHoverCard as="section" unstyled className="grid gap-6 rounded-[28px] border border-border bg-card p-5 sm:p-6 md:grid-cols-[minmax(0,0.44fr)_minmax(0,1fr)]">
      <div className="mx-auto w-full max-w-[310px] self-start rounded-[22px] bg-gradient-to-br from-emerald-50 to-slate-200 p-5"><AnimatedBook {...book} /></div>
    ...
    <section className="mb-12 border-t border-border pt-8"><h2 className="mb-6 text-2xl font-extrabold">Related Books</h2>{relatedError ? <CatalogUnavailable title="Buku terkait belum dapat dimuat" /> : related.length ? <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">{related.map(item => <BookHoverCard unstyled key={item.id} className="min-w-0"><Link href={`/books/${encodeURIComponent(item.id)}`} className="block h-full min-w-0 rounded-[20px] border border-border bg-card p-3 transition-colors hover:border-cyan-300 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-cyan-300"><AnimatedBook {...item} /><h3 className="mt-3 text-sm font-bold">{item.title}</h3><p className="mt-2 text-xs text-palette-slate-400">{item.author}</p><p className="mt-3 text-xs text-yellow-700 dark:text-yellow-400">★ {item.rating.toFixed(1)}</p></Link></BookHoverCard>)}</div> : <p className="text-palette-slate-400">Belum ada buku lain dalam kategori ini.</p>}</section>
  </>;
}
